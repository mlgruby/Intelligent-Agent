...
public class VacuumAgentOrange extends VacuumAgent {
private static final long serialVersionUID = -565563161972920536L;
	...
	public VacuumAgentOrange(AgentBrain brain, int x, int y, String direction, String type) {
		super(brain);
		cleanerType = type;
		curLocationX = x;
		curLocationY = y;
            ...
            if (action.getActionType().equals(VacuumActionType.SENSING_ACT.toString())) {
            	...
            	for (int i = 0; i < obs.length; i++)
            		if (obs[i].startsWith("agent")) {
						...
						String ag[] = obs[i].substring(5, obs[i].length() - 1).split(",");
						if(ag[0].equals("ORANGE")) { // add only orange dirt
...
//								System.out.println("X: " + j + " Y: " + i);
			...
			if (isGoalReached) {
				// if orange dirt then reset the colour and clean orange dirt
